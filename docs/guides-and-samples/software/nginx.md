...
- Pre-compressing static files. Our example uses gzip compression with the [`gzip_static` module](https://nginx.org/en/docs/http/ngx_http_gzip_static_module.html). Or if you use Nginx Plus, you can use the [`brotli_static` module](https://docs.nginx.com/nginx/admin-guide/dynamic-modules/brotli/) for better compression ratios.
- Using the [`sendfile`](https://nginx.org/en/docs/http/ngx_http_core_module.html#sendfile) and [`sendfile_max_chunk`](https://nginx.org/en/docs/http/ngx_http_core_module.html#sendfile_max_chunk) directives to optimize file transfers
- Using the [`tcp_nopush`](https://nginx.org/en/docs/http/ngx_http_core_module.html#tcp_nopush) directive to improve network performance
- Allocating minimal resources since Nginx is lightweight
...
```nginx title="nginx.conf" {5-8,10-11,16-19}
server {
    listen      80;
    server_name _;
...
Instead, use [services](../../resources/services.mdx) provided by lttle.cloud for these functionalities. They have been optimized for serverless environments and will provide better performance and reliability.
